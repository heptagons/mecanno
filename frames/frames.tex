...
for a given maximum strips length. The report reject the triangles where $z_1 > 1$ and when $z_3$ is not a square-free. Next example report list all the triangles $z_3 = 7, max=15$ so the software filters are $c < a+b$, $a\leq d\leq max$, $b\leq e\leq max$, $c\leq max$:

\boxed{TODO: Write a report in tex format to paste here!!!}
...
Figure \ref{fig:surd7} show four triangles reported.
