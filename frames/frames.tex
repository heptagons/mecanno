...
First we calculate the angle $\theta = \angle{ACB}$ defining variables $m,n$:
\begin{align}
(m, n) &\equiv (a^2 + b^2 - c^2, 2ab) \quad | m | \le n \label{eq:mn}\\
\cos\theta &= \frac{a^2 + b^2 - c^2}{2ab} = \frac{m}n \\
\sin\theta &= \sqrt{1-\cos^2\theta} = \frac{\sqrt{n^2 - m^2}}n
\end{align}

Then we calculate the angle $\phi = \angle{DFE}$ defining variable $o,p$:
\begin{align}
(o, p) &\equiv (d^2 + e^2 - f^2, 2de) \quad | o | \le p \label{eq:op}\\
\cos\phi   &= \frac{d^2 + e^2 - f^2}{2de} = \frac{o}p \\
...
\subsubsection{$(B_x, B_y)$}
We use the triangle above $M(a,b,c)$ and make the extension $u = |y_1|$ and obtain $(B_x, B_y)$. We already calculated angle $\theta=\angle{ACB}$ for the original triangle pair using the variables $m,n$ of equation \ref{eq:mn}.
\begin{align}
