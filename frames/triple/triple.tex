...
\end{tikzpicture}
\caption{
At the left we have the triple frame (three angles $\theta$) with the strips $a,b,c,d,e$.
At the right we use two frames to build a regular polygon of side $a$ extending strips $b,c,d$ to fix everthing. This construction is possible only when $a > b,c$.
}
\label{fig:triple}
\end{figure}
...
red strips and perimeter the same. The original \textbf{triple frame} $a,b,c,d,e$ irregular pentagon 
