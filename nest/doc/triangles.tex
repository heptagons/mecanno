...
Triangles$(\sqrt{a},b,c)$ have three sides with lengths $\sqrt{a}$, $b$ and $c$ where $a, b, c \in \bbb N$
and $a$ is squarefree. We have:
\begin{align}
...
In this case $\sqrt{a} = 2\sqrt{6}$ so $a = 24$.
...
Then we remove the cells that don't satisfy the condition $a < (b+c)^2$:
...
\begin{figure}[htp]
\centering
\includegraphics[scale=1]{tslurA}
\caption{Triangles with sides $a = 2\sqrt{6} > b \ge c$}
\label{tslurA}
\end{figure}
