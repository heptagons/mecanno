...
// rQ reduces the quotient (± num0 ± num1 ± num2 ± ... ± numN) / den
func (a *N32s) rQ(den N, nums ...int64) (den32 N, n32s []int32, overflow bool) {
	if den == 0 {
	...
	if allNum0 {
		return 1, make([]int32, len(nums)), false
	}
	// nsMinPos points to the smallest num to reduce primes use
	var minPos int
	var min N
	ns := make([]N, len(nums))
	for p, n := range nums {
...
func (a *N32s) Reduce(p ...int64) (r []int32, overflow bool) {
	...
	if n == 2 {
		...
		if g1, h1, overflow := a.roi(p[2], p[3]); overflow {
			...
			return a.Reduce(p[0], p[1] + int64(g1))
		} else if e1, fg, overflow := a.roie(p[0], p[1], int64(g1)); overflow { // reduced 
		...
		if mnop, overflow := a.Reduce(p[4], p[5], p[6], p[7]); overflow {
			...
			return a.Reduce(p[0], p[1] + int64(m1), p[2], p[3])

		} else if kl, overflow := a.Reduce(p[2], p[3]); overflow {
			// k√l overflow
			return nil, true

		} else if i1, jkm, overflow := a.roie(p[0], p[1], int64(kl[0]), int64(m1)); overflow { // reduced 
			// reduction i√(j+k+m1) overflow
			return nil, true
		} else {
			var l1, n1, o1, p1 int32
			if len(kl)   > 1 { l1 = kl[1]   }
...
func (a *N32s) roi(o, i int64) (o32, i32 int32, overflow bool) {
	if o == 0 || i == 0 {
		return 0, 0, false
	}
...
func (a *N32s) roie(o int64, is ...int64) (o32 int32, i32s []int32, overflow bool) {
	ins0 := true
	for _, i := range is {
		if i != 0 {
			ins0 = false
