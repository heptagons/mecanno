...
\coordinate(A) at (0,0);
\coordinate(B) at (#1,0);
\coordinate(K) at (#1+\cx, \cy);
\coordinate(H) at (-\bx,\by);
\coordinate(L) at (-\bx-\dxx,\by+\dyy);
\coordinate(AA) at (-\ca,0);
\coordinate(AC) at (-\ca-\cax,+\cay);
\coordinate(CC) at (-\ca-\cax-\ax, +\cay+\ay);
\coordinate(CCC) at (-\cax, +\cay);
\draw[blue,fill=blue](K)
 -- node[above]{$#3$} (B) circle(5pt)
 -- node[above]{$#1$} (A) circle(5pt)
 -- node[above]{$#2$} (H) circle(5pt)
 -- node[right]{$#4$} (L) circle(5pt)
 -- node[below]{$#5$} (K) circle(5pt)
 ;
\draw[black,fill=black](A)
 -- node[below]{$\ca$} (AA) circle(5pt)
 -- node[below]{$\ca$} (AC) circle(5pt)
 -- node[below]{$#1$} (CC) circle(5pt) (AC)
 -- node[above]{$\ca$} (CCC) circle(5pt)
 ;
}
...
\begin{scope}[scale=0.15]
 ...
 \begin{scope}[shift={(60,0)}] %row-1 col-1
