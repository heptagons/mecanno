...
\title{\textbf{Meccano fox unit}}
\author{https://github.com/heptagons/meccano/units/fox}
\date{}
...
\maketitle
\begin{abstract}
Meccano fox unit is a group of five meccano\meccanoref strips intended to be a
base to build equilateral polygons. The unit resembles a fox face with two big pointing ears.
Originally the unit was used to build a regular pentagon\footnote{
\href{https://github.com/heptagons/meccano/tree/main/penta/pentagons.pdf}{Meccano pentagons}	
} and here we explore more polygons.
We conjecture the fox unit permits to build a single pentagon, infinite hexagons,
...
According to the program results, we conjecture that including the \textbf{fox unit} exist:
...
\newcommand{\foxfacehexagon}[6]{ % three foxfacehex units
 \begin{tikzpicture}
 \def\a{#3};\def\b{#4};\def\c{#5};\def\d{#6};\def\bc{#4+#5}
 \pgfmathsetmacro\max{max(#3,#4+#5)} % max(a,b+c)
...
\caption{Hexagon side 4 with three fox semi-units (4,1,3,7).}
\end{figure}
