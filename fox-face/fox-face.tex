...
\newcommand{\foxface}[6]{
 ...
 \pgfmathsetmacro\angleB{acos((#5*#5 + #6*#6 - (#3+#4)*(#3+#4))/(2.0*#5*#6))}
 \def\angleA{acos{0.5}}
 \begin{scope}
  \meccanostrip[0000FF]{\a}{#1}{#2} %a
 \end{scope}
 \begin{scope}[shift={(#1*\a,0)},rotate=\angleA]
  \meccanostrip[00FF00]{\a}{#1}{#2} %a > b+c
  \begin{scope}[shift={(#1*\bc,0)},rotate=180-\angleB]
   \meccanostrip[FF0000]{\d}{#1}{#2}; %d
  \end{scope}
 \end{scope}
 \end{tikzpicture}
}
...
\centering
\scalebox{0.5}{
 \foxface{1}{5pt}{3}{4}{8}{11} % size,factor,a,b,c,d
}
\caption{Fox-face(3,4,8,11)}
\end{figure}
...
\centering
\scalebox{0.5}{
 \foxface{1}{5pt}{4}{1}{3}{7} % size,factor,a,b,c,d
}
\caption{Fox-face(4,1,3,7)}
\end{figure}

\begin{figure}[h!]
\centering
\scalebox{0.4}{
 \foxface{1}{5pt}{9}{1}{6}{14} % size,factor,a,b,c,d
}
\caption{Fox-face(9,1,6,14)}
\end{figure}
